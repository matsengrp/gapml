...
def main():
    ...
    parser.add_argument('--outbase', type=str, help='output basename for phylip file and abundance weight file')
    ...
    for record in SeqIO.parse(args.fastq, 'fastq'):
        seq_events = [event.group(0) for event in re.compile('[0-9]*:[0-9]*:[acgt]*').finditer(record.description)]
        if str(record.seq) not in processed_seqs:
            ...
            if processed_seqs[str(record.seq)][1] != seq_events:
                warnings.warn('identical sequences have different event calls: {}, {}\nsequence: {}'
                              .format(processed_seqs[str(record.seq)][1], seq_events, str(record.seq)))
    ...
    with open(args.outbase + '.phy', "w") as f1, open(args.outbase + '.abundance', "w") as f2:
